...
public:
	...
	//in loc sa se calculeze diferenta intre intensitati, se calculeaza un cost care tine cont
	// de unghiul dintre 2 normale
	//ex:  N1   N2   N3
	//     N4   N5   N6
	//     N7   N8   N9
	...
	struct RegionSummary {
		float I_avg = 0.f;
		float D_avg = 0.f;
		glm::vec3 N_avg = glm::vec3(0, 0, 1);
		int count = 0;
	};

	static void segmentBFSFromSeeds(const uchar* grayscaleData, const cv::Vec4f* pointCloudData, const cv::Vec4f* normalMeasureData,
	...
	static void labelsToColorImage(const std::vector<int>& labels, int width, int height, cv::Vec4b* outRGBAMatrix);
};
